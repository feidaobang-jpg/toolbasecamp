...
from life_plans import PLAN_KINDS, LIFE_PLANS_PROMPT_REV, LIFE_PLANS_GUEST_OK
...
assert LIFE_PLANS_PROMPT_REV == 9, 'disk prompt_rev=%s' % LIFE_PLANS_PROMPT_REV
assert LIFE_PLANS_GUEST_OK is True
...
for i in 1 2 3 4 5 6 7 8; do
  ...
  if echo "$HEALTH" | grep -q '"life_plans_api":true' \
    ...
    && echo "$HEALTH" | grep -q '"life_plans_prompt_rev":9' \
    && echo "$HEALTH" | grep -q '"life_plans_guest_ok":true' \
...
if [[ "$OK" != "1" ]]; then
  echo "FAILED: health missing life_plans_ready / prompt_rev=9 in process memory"
...
echo "OK: life-plans API is live on :8001 (prompt_rev=9)"
