#!/bin/bash
# One-shot fix when /api/recipe/generate returns 404 (stale API or missing files).
set -euo pipefail
...
grep -n "recipe/generate" "$APP_DIR/main.py" || { echo "ERROR: main.py has no recipe route"; exit 1; }
