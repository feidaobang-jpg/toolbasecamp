...
assert RENT_DUE_DAY_MAX >= 31, 'stale due day max: ' + str(RENT_DUE_DAY_MAX)
...
echo "$HEALTH" | grep -q '"records_rents":true' || {
  echo "FAILED: health records_rents is not true (stale process?)"
  journalctl -u toolbasecamp-api -n 40 --no-pager || true
  exit 1
}
echo "$HEALTH" | grep -q '"records_online_games":true' || {
  echo "FAILED: health records_online_games is not true (stale process?)"
  journalctl -u toolbasecamp-api -n 40 --no-pager || true
  exit 1
}
echo "$HEALTH" | grep -q '"records_online_draft":true' || {
  echo "FAILED: health records_online_draft is not true (stale process?)"
  journalctl -u toolbasecamp-api -n 40 --no-pager || true
  exit 1
}
...
echo "$HEALTH" | grep -q '"records_rent_due_max":31' || {
  echo "FAILED: health records_rent_due_max!=31"
  journalctl -u toolbasecamp-api -n 40 --no-pager || true
  exit 1
}
...
curl -s http://127.0.0.1:8001/openapi.json | grep -q '/records/rents' || {
  echo "FAILED: openapi missing /records/rents"
  journalctl -u toolbasecamp-api -n 40 --no-pager || true
  exit 1
}
curl -s http://127.0.0.1:8001/openapi.json | grep -q '/records/online-games' || {
  echo "FAILED: openapi missing /records/online-games"
  journalctl -u toolbasecamp-api -n 40 --no-pager || true
  exit 1
}
