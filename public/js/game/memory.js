...
function tr(k, p) { return typeof window.t === 'function' ? window.t(k, p) : k; }
    ...
    function onFlip() {
        ...
        if (a.dataset.emoji === b.dataset.emoji) {
            a.classList.add('is-matched'); b.classList.add('is-matched'); play('match'); matched++; hud(); lock = false;
            if (matched === 8) play('win'); setStatus(tr('tools.memory.win', { n: moves }), 'is-win');
        } else {
            setTimeout(function () {
