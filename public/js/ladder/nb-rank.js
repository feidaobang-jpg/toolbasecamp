...
(function () {
  ...
  function setMeta() {
    ...
    if (!meta) return;
    var parts = [];
    if (state.updatedAt) parts.push(tr('tools.ladderNbGpuRank.updated', '更新') + '：' + state.updatedAt.replace('T', ' ').replace('Z', ' UTC'));
  ...
  function render() {
    ...
    list.forEach(function (it, idx) {
      ...
      row.innerHTML =
        '<div class="nb-rank-pos">' + (idx + 1) + '</div>' +
        '<div class="nb-rank-main">' +
          '<div class="nb-rank-name"></div>' +
          '<div class="nb-rank-sub"></div>' +
          '<div class="nb-rank-bar-wrap"><div class="nb-rank-bar ' + brandClass(it.brand) + '" style="width:' + pct + '%"></div></div>' +
        '</div>' +
        '<div class="nb-rank-score">' + (it.perf_rating != null ? Number(it.perf_rating).toFixed(1) : '-') + '</div>';
  ...
  function load() {
    ...
    fetch(apiBase() + '/nbcheck/nb_gpu', { headers: { Accept: 'application/json' }, cache: 'no-store' })
      ...
      .catch(function (err) {
        if (root) {
          root.innerHTML = '<div class="nb-rank-error">' +
