"""HappyHorse 1.1 text-to-video (DashScope) — async submit + poll + proxy download.
...
HH_MODEL = (os.environ.get("HAPPYHORSE_T2V_MODEL") or "happyhorse-1.1-t2v").strip()
...
def _ensure_charged(meta: Dict[str, Any], user: dict, task_id: str) -> Optional[float]:
    ...
    try:
        new_bal = try_charge(
            ...
            int(user["id"]),
            charge,
            reason="happyhorse_t2v",
            meta={
                "taskId": task_id,
                "duration": meta.get("duration"),
                "resolution": meta.get("resolution"),
                "ratio": meta.get("ratio"),
                "model": meta.get("model") or HH_MODEL,
...
async def hh_t2v_task(task_id: str, user: dict = Depends(_user)):
    ...
    return {
        ...
        "proxy_url": f"/happyhorse/t2v/proxy/{task_id}" if video_url or status == "SUCCEEDED" else None,
        "message": message,
...
async def hh_t2v_proxy(task_id: str, user: dict = Depends(_user)):
    ...
    # Charge if somehow not yet charged
    ...
    return StreamingResponse(
        ...
        headers={"Content-Disposition": 'attachment; filename="happyhorse-t2v.mp4"'},
    )
